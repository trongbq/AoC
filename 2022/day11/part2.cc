...
int main (int argc, char *argv[]) {
    ...
    if (my_file.is_open()) {
        ...
        // need this to help with reducing worry level
        int factor = 1;
        for (int i = 0; i < monkeys.size(); i++) {
            factor *= monkeys[i].test_condition;
        }
        ...
        for (int r = 1; r <= 10000; r++) {
            for (int i = 0; i < monkeys.size(); i++) {
                ...
                for (auto item : monkeys[i].items) {
                    long worry_level = monkeys[i].operation.exec(item);
                    worry_level %= factor; // we don't need to keep huge worry level
            ...
            if (r == 1 || r == 20 || r % 1000 == 0) {
                cout <<  "Round " << r << endl;
                for (int i = 0; i < monkeys.size(); i++) {
                    cout << "Monkey " << i << " inspected items " << monkeys[i].inspection_count << " times" << endl;
                }
                cout << endl;
            }
        }
